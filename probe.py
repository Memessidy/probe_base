...
class PostCardMaker:
    def __init__(self, name, template=None, font_path=None):
        ...
        if font_path is None:
            self.font_path = "ofont_ru_DS Eraser2.ttf"
        else:
    ...
    def make(self, resize=False, out_path=None):
        ...
        draw.text((10, y), message, font=font, fill=ImageColor.colormap['aquamarine'])

        y = im.size[1] - 20 - font.size
        message = f"С ужасным праздником тебя!"
        draw.text((10, y), message, font=font, fill=ImageColor.colormap['aquamarine'])

        im.save(out_path if out_path else 'probe.jpg')
        # im.show()
        print(f"Post card saved at {out_path}")
